...
python scripts/auto_feedback_loop.py \
  ...
  --operation cosmosdb_create \
  --command "az cosmosdb create --name {{user.account_name}} --resource-group {{user.resource_group}} ..." \
  --desired-state '{"provisioningState": "Succeeded"}' \
  [--dry-run] [--trace-id <uuid>]
```
- **Non-risky operations** (cosmosdb_create, sql_database_create): auto-feedback loop active
- **Risky operations** (delete): always bypass loop and require explicit human confirmation
- Healing policy: see [`scripts/self_healing/cosmos_heal.json`](../../scripts/self_healing/cosmos_heal.json)
