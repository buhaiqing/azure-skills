...
python scripts/auto_feedback_loop.py \
  ...
  --operation redis_create \
  --command "az redis create --name {{user.redis_name}} --resource-group {{user.resource_group}} ..." \
  --desired-state '{"provisioningState": "Succeeded"}' \
  [--dry-run] [--trace-id <uuid>]
```

- **Non-risky operations** (redis_create): auto-feedback loop active
- **Risky operations** (delete): always bypass loop and require explicit human confirmation
- Healing policy: see [`scripts/self_healing/redis_heal.json`](../../scripts/self_healing/redis_heal.json)
